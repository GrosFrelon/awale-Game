...
int appliquerCoup(int numeroJoueur, int position, jeu_t* jeu){
    ...
    if(position<1 || position>6){return 1;}
    if(numeroJoueur == 0){
        position = position-1;
    }
    if(numeroJoueur == 1){
    ...
    //On sème les graines
    //nbGraineRecup%12 pour compenser les sauts de la case ou l'on a recup les graines
    posDemarageSemage = position + jeu->rotation;
    for(int i = posDemarageSemage ; i<(nbGrainesRecuperees + nbGrainesRecuperees/12 + posDemarageSemage) ; i = i + jeu->rotation){ 
        indiceParcours = ((i%12) + 12)%12;  //toujours un nombre positif entre 0 et 12
    ...
    while( (jeu->plateau[indiceParcours]==2 || jeu->plateau[indiceParcours]==3) && positionCampAdverse(indiceParcours, numeroJoueur)){
        augmenterScore(numeroJoueur, jeu->plateau[indiceParcours], jeu);
        jeu->plateau[indiceParcours] = 0;
        indiceParcours -= jeu->rotation;  //retourne en arrière du sens de rotation pour recup les graines
    }
    return 0;
